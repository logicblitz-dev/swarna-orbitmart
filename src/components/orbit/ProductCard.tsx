...
export default function ProductCard({ product }: { product: Product }) {
  ...
  const onHeart = (e: MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    e.stopPropagation();
    toggle(product);
  };
  ...
  return (
    <motion.div layout className="[perspective:1000px]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ duration: 0.4 }}>
      <Link
        to="/product/$id"
      ...
      >
        ...
        >
          ...
          <button
            ...
            className={`absolute top-3 left-3 z-20 w-9 h-9 rounded-full glass flex items-center justify-center transition ${liked ? "heart-pop" : ""}`}
          >
            <Heart
              className={`w-4 h-4 transition ${liked ? "fill-[#ff3b8b] text-[#ff3b8b]" : "text-white/70"}`}
              style={liked ? { filter: "drop-shadow(0 0 8px #ff3b8b)" } : undefined}
            />
          </button>
